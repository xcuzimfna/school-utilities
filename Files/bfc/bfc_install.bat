...
:gotAdmin
    ...
    for /l %%f in (0 1 100) do (
        call :drawProgressBar %%f "preparing for installation"
        if exist C:\Windows\System32\bfc.bat goto ai
    )
    for /l %%f in (100 -1 0) do (
        call :drawProgressBar %%f "installing..."
    )
    for /l %%f in (0 5 100) do (
        call :drawProgressBar !random! "copying files..."
    )
    ...
    call :drawProgressBar 0 "verifying files..."
    for /l %%f in (0 1 100) do (
...
copy bfc.bat C:\Windows\System32 && (
